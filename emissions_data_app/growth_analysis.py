...
def find_multiplier(original_data, mult_columns):
    ...
    if len(mult_columns) != 2:
        raise ValueError('mult_columns must contain two columns')
...
def grouped_growth_rate_multipliers(original_data, mult_columns, number_of_groups):
    ...
    growth_quantile(co2_data, ['gdp', 'co2'], 4).
    ...
    # iterate through the two passed columns and add a suffix to the name to match the output of
    # sg.extract_growth_rates_from_summary_df
    for col in mult_columns:
        col_growth_names.append(str(col + ' % growth'))

    # run the find_multiplier function to find the growth rate data, calculate the multipliers, and copy to a df
    ...
    grouped_growth_df = multiplier_df
    grouping_column_name = col_growth_names[0]
    grouped_growth_df['Growth Rate Group'] = pd.qcut(grouped_growth_df[grouping_column_name], q=number_of_groups,
                                              labels=range(1, number_of_groups + 1))

    return grouped_growth_df
...
def find_grouped_mean_multiplier(original_data, mult_columns, number_of_groups):
    ...
    # grouped_multiplier_df.to_excel(r'C:\Users\chille\Python\historical-co2-data\emissions_data_app\grouped_multiplier.xlsx')
...
def find_grouped_multiplier_statistics(original_data, mult_columns, number_of_groups):
    """
    Takes the full data set, chosen columns, and desired number of groups, returns the summary statistics of multipliers
    for each group of growth rates of the first passed column

    :param original_data: pass the original, unaltered owid co2 data dataframe that was downloaded from the owid GitHub
    :param mult_columns: the names of the columns for which you want to calculate the mean growth rate multiplier
    :param number_of_groups: the number of groups you want to split the data into. The set will be divided into chosen
     number of groups using the number_of_groups - 1 quantiles of the growth rates of the first passed column
    :return: the summary statistics of the multipliers for countries in each equal group based on
    growth rate of denominator
    """
    ...
    # create dictionary that will contain {group : multiplier stats}
    multiplier_statistics_dict = {}

    # iterate through each group using number_of_groups and calculate multiplier stats for each group
    for group in range(1, number_of_groups + 1):
        group_df = grouped_multiplier_df[grouped_multiplier_df['Growth Rate Group'] == group]
        group_multiplier_statistics = group_df[mult_col_name].describe()
        multiplier_statistics_dict[group] = group_multiplier_statistics
